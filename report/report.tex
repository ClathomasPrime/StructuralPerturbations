...
\begin{proof}
  ...
  \begin{algorithm}[H]
    \begin{algorithmic}[1]
      \Function{Listify}{$t$}
        ...
        \EndWhile
        \State{\{$t$ is in degenerate list form\}}
      \EndFunction
    \end{algorithmic}
  \end{algorithm} 
  ...
  % Proof: the first loop clearly terminates (as $\rho^{-1}$ decreases the depth
  % of the right subtree) and leaves $t$ with a null right
  ...
  % (as $\theta$ decreases the depth of the left child's right child)
